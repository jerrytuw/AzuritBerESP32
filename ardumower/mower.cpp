...
volatile unsigned long rpmLeftDuration = 100000; // for startup
volatile unsigned long rpmRightDuration = 100000;
volatile unsigned long motorLeftLastTime = 0;
volatile unsigned long motorRightLastTime = 0;
...
Mower::Mower() {
  ...
  motorOdoAccel =     3000; //Time for accel from 0 to 100% in ms
  motorSpeedMaxRpm =  24;   // motor wheel max RPM (WARNING: do not set too high, so there's still speed control when battery is low!)
  motorSpeedMaxPwm =  100;  // motor wheel max Pwm  (8-bit PWM=255, 10-bit PWM=1023)
  motorPowerMax =     50;    // motor wheel max power (Watt)
  motorSenseRightScale = 0.5; // normal is 1.536 motor right sense scale (mA=(ADC-zero)/scale)
  motorSenseLeftScale = 0.5; // normal is 1.536 motor left sense scale  (mA=(ADC-zero)/scale)
  ...
  motorTickPerSecond = 45; // use to compute the maxodostate duration and computed on the calibration motor
  ...
  odometryTicksPerRevolution = 90;   // encoder ticks per one full resolution
  odometryTicksPerCm = 1.41;  // encoder ticks per cm
...
// odometry signal change interrupts

void IRAM_ATTR OdometryLeftISR() {
  unsigned long delta = millis() - motorLeftLastTime;
  ...
  {
    rpmLeftDuration = millis() - motorLeftLastTime;
    motorLeftLastTime = millis();
    // in 60000ms/45 or 90 pulses per rot = 1Upm => 1333.33 or 666.66 ms per pulse
    //    if (delta) rpmLeftDuration = (60000. / (float)(TICKS_PER_REVOLUTION)) / (float)delta;
  }
  odomTicksLeft++;
  if (robot.motorLeftPWMCurr >= 0) robot.odometryLeft++; else robot.odometryLeft--;
}

void IRAM_ATTR OdometryRightISR() {
  ...
  {
    // in 60000ms/45 pulses per rot = 1U => 1333.33 ms per rpm
    rpmRightDuration = (millis() - motorRightLastTime);
    motorRightLastTime = millis();
  }
  odomTicksRight++;
...
void Mower::setup() {
  ...
  {
    Serial.println("Error - failed to initialise EEPROM area");
  }
  else Serial.println("EEPROM area initialized");
#endif
  Flash.test();
  ...
  */
  ...
  if (esp8266Use) {
    ...
    rc.initSerial(&FakeSerial, BLUETOOTH_BAUDRATE);
  ...
  // Due interrupts
  attachInterrupt(pinOdometryRight, OdometryRightISR, CHANGE); // note: more precise than count but still oops depending on direction
...
int Mower::readSensor(char type) {
  ...
  switch (type) {
    ...
    // motors------------------------------------------------------------------------------------------------
    case SEN_MOTOR_MOW: return 0; //return ADCMan.getValue(pinMotorMowSense); break;
